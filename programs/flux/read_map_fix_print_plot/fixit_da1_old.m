...
disp('PISTON_MISOBOB_2019_fixit_da1')

% remove bad periods of ship data
ii = jdy<150.2;
dir_scs(ii) = NaN; rwdir_scs(ii) = NaN;
U_scs(ii) = NaN; rwspd_scs(ii) = NaN;
rwspd_im_std(ii) = NaN; rwdir_im_std(ii) = NaN;
rs_im(ii) = NaN; rl_im(ii) = NaN;
sog_im(ii) = NaN; cog_im(ii) = NaN;
sog_im(sog_im>10) = NaN; cog_im(sog_im>10) = NaN;
...
plot(jdy,U,'r.',jdy,U_scs,'b.'); grid; xlim([stdt endt]);
legend('NOAA','SCS','location','eastoutside');
ylabel('True Wspd m/s'); xlabel('DOY 2019');
...
[xbins, ybins1, ~, SD1, ~] = binave2(U,U_scs,2,0,18);
...
figure('position',[1,1,800,475]);
plot(jdy,dir,'r.',jdy,dir_scs,'b.'); grid; xlim([stdt endt]);
ylim([0,360]); yticks([0,90,180,270,360]);
ylabel('True Wdir, deg'); xlabel('DOY 2019');
legend('NOAA','SCS','location','eastoutside');
...
p1 = plot(jdy,reldir,'r.',jdy,rwdir_scs,'b.'); grid; xlim([stdt endt]);
ylim([-180,180]); yticks([-180,-90,0,90,180]);
ylabel('Rel Wdir, deg'); xlabel('DOY 2019');
legend(p1,{'NOAA','SCS'},'location','eastoutside');
...
P_im(P_im<1000) = NaN;
figure('position',[1,1,800,475]);
plot(jdy,press,'r.',jdy,P_im,'b.',jdy,Pmb_wxt,'g.'); xlim([stdt endt]); grid;
ylabel('Sea Level pressure, mb'); xlabel('DOY 2019');
legend('NOAA','SCS','WXT','location','eastoutside');
...
[bpbinNOAA, bpbinSCS, ~, SDbpSCS, ~] = binave2(press,P_im,2,1000,1014);
...
legend([p1 p2],'SCS','WXT','location','best');
ylabel('Sea Level BP, SCS, mb'); xlabel('Sea Level BP, NOAA, mb');
...
disp(['Mean Diff BP, NOAA-Ship = ',sprintf('%6.2f',nanmean1(press-P_im))]);
...
figure('position',[1,1,800,475]);
plot(jdy,tsnk,'r',jdy,tsg,'b',jdy,rosr,'g'); grid;
xlim([stdt,endt]); ylim([minx,maxx]);
ylabel('SST, C'); xlabel('DOY 2019');
legend('Sea Snake','SCS','location','eastoutside');
...
ii = isfinite(tsnk) & isfinite(tsg);
[xbins0, ybins0, ~, SD0, ~] = binave2(hDay(ii),tsnk(ii),2,0,24);
[xbins1, ybins1, ~, SD1, ~] = binave2(hDay(ii),tsg(ii),2,0,24);
...
legend([p1 p2],{'Sea Snake','SCS'},'location','eastoutside');
...
figure('position',[1,1,800,475]);
plot(jdy,rh,'r.',jdy,rh_im,'b.',jdy,RH_wxt,'g.');
ylabel('RH, %'); xlabel('DOY 2019'); grid; xlim([stdt endt]);
legend('PSD','SCS','WXT','location','eastoutside');
...
figure('position',[1,1,800,475]);
plot(jdy,ta,'r.',jdy,ta_im,'b.',jdy,Tair_wxt,'g.');
ylabel('T air, C'); xlabel('DOY 2019'); grid; xlim([stdt endt]);
legend('PSD','SCS','WXT','location','eastoutside');
...
qa_im = qair_p(ta_im,rh_im,P_im);
qa_wxt    = qair_p(Tair_wxt,RH_wxt,Pmb_wxt);
figure('position',[1,1,800,475]);
plot(jdy,qa,'r.',jdy,qa_im,'b.',jdy,qa_wxt,'g.'); grid; xlim([stdt endt]);
ylabel('Q air, g/kg'); xlabel('DOY 2019');
legend('PSD','SCS','WXT','location','eastoutside');
...
qs_tsg =qsea_p(tsg,P_im);
figure('position',[1,1,800,475]);
plot(jdy,qse,jdy,qs_tsg); grid; xlim([stdt endt]);
ylabel('Qsea, g/kg'); xlabel('DOY 2019');
legend('Sea Snake','SCS','location','eastoutside');
...
figure('position',[1,1,800,475]);
plot(jdy,rs,'r.',jdy,rs_im,'b.'); grid; xlim([stdt endt]);
ylabel('Solar Flux, W/m^2'); xlabel('DOY 2019');
legend('PSD','SCS','location','eastoutside');
...
figure('position',[1,1,800,475]);
plot(jdy,rl,'r.',jdy,rl_im,'b.'); grid; xlim([stdt endt]);
ylabel('IR Flux, W/m^2'); xlabel('DOY 2019');
legend('PSD PIR1','SCS','location','eastoutside');
...
figure('position',[1,1,800,475]);
plot(jdy,rain,'r',jdy,rain_wxt,'b'); grid; xlim([stdt endt]);
ylabel('Rain, mm/hr'); xlabel('DOY 2019');
legend('ORG','WXT','location','eastoutside');
...
figure('position',[1,1,800,475]);
plot(jdy,sog,'r.',jdy,sog_im,'b.'); grid; xlim([stdt endt]);
ylabel('SOG, m/s'); xlabel('DOY 2019');
legend('NOAA','SCS','location','eastoutside');
...
figure('position',[1,1,800,475]);
plot(jdy,cog,'r.',jdy,cog_im,'b.'); grid; axis([stdt,endt,0,360]);
yticks([0,90,180,270,360]); ylabel('COG, deg'); xlabel('DOY 2019');
legend('NOAA','SCS','location','eastoutside');
...
ylabel('Heading'); xlabel('DOY 2019');
...
ylabel('Std Dev Heading'); xlabel('DOY 2019');
...
map_PISTON_2019(Lon,Lat,LatLonLim,LegNo)
if savefigs
