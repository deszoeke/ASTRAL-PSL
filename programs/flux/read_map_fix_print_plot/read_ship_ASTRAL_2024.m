function [ship] = read_ship_ASTRAL_2024(dfl,ddd,hhh,PosLims,SCS_adj,zpim, zq_ship)
...
%%% starboard bow sea
%     Fsea          fluorometer dry value micrograms/L
%     Csea          C SBE45 (S/m)
%     ssea          S SBE45 (psu)
%     tsea_int      T of internal sensor SBE45 (C)

%%% aft porside sea
%     Fsea2          fluorometer dry value micrograms/L
%     Csea2          C SBE45 (S/m)
%     ssea2          S SBE45 (psu)
%     tsea2_int      T of internal sensor SBE45 (C)
...
the_fields = {'lat';'lon'; 'sog';'cog';'hed';'rspd';'rdir';'wspd';'wdir';'rspd2';'rdir2';'wspd2';'wdir2';...
    'rspd3';'rdir3';'wspd3';'wdir3';'ta';'rh';'pa';'tsea_in';'tsea';'ssea';'lw_dn';'sw_dn';'PAR'};
fs = length(the_fields);

% these fields were then calculated
full_fields = vertcat(the_fields,{'qa';'psealevel'; 'pa_at_zq';'sogN';'sogE'});
...
if exist(dfl,'file')==2
    ...
    N=29; % columns of data in raw file
    temp = []; ship_raw = [];
    while ~feof(flist)
        try


            temp = textscan(flist,['%2f%2f%3f %fN %fE',repmat('%f',1,24),' %*[^\n]'],'delimiter',',','headerlines',1,'emptyvalue',NaN);
...
%     disp(imsec(1));
    ...
    for i = 1:N-3
       eval([the_fields{i} ' = ship_raw(:,i+3);'])
    end
    ...
    %convert lat/lon from degs.minutes to dec degrees and clean up
    lat =     lat./100;
    lon =     lon./100;
    latmin = (lat-floor(lat))*100;
    lat = floor(lat)+latmin/60;
    lonmin = (lon-floor(lon))*100;
    lon = floor(lon)+lonmin/60;
...
%     paccum(paccum < 0) = NaN;    
    ...
    tsea(tsea>50) = NaN;
    tsea(tsea<-2) = NaN;
    tsea_in(tsea_in>50) = NaN;
    tsea_in(tsea_in<-2) = NaN;
%     ssea2(ssea2>40) = NaN;
%     ssea2(ssea2<25) = NaN;
%     tsea2(tsea2>50) = NaN;
%     tsea2(tsea2<-2) = NaN;
...
%     qa(qa>25) = NaN;
    ...
    [wspd2,~] = despike2(wspd2);
    [rspd2,~] = despike2(rspd2);
    wdir2 = unwrap(wdir2*pi/180); % unwrap and convert to radians
    [wdir2,~] = despike2(wdir2);
    rdir2 = unwrap(rdir2*pi/180); % unwrap and convert to radians
    [rdir2,~] = despike2(rdir2);


    [wspd3,~] = despike2(wspd3);
    [rspd3,~] = despike2(rspd3);
    wdir3 = unwrap(wdir3*pi/180); % unwrap and convert to radians
    [wdir3,~] = despike2(wdir3);
    rdir3 = unwrap(rdir3*pi/180); % unwrap and convert to radians
    [rdir3,~] = despike2(rdir3);
...
%     [qa,~] = despike2(qa);
    ...
    wdir2 = (wdir2*180/pi);
    wdir2 = mod(wdir2 + 360, 360);
    wdir3 = (wdir3*180/pi);
    wdir3 = mod(wdir3 + 360, 360);
    
    %%% convert rdir to degrees: +/- 180
    rdir = (rdir*180/pi);
    rdir(rdir>180) = rdir(rdir>180)-360; 
    rdir2 = (rdir2*180/pi);
    rdir2(rdir2>180) = rdir2(rdir2>180)-360; 
    rdir3 = (rdir3*180/pi);
    rdir3(rdir3>180) = rdir3(rdir3>180)-360; 
