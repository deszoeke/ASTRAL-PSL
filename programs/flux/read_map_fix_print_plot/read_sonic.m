...
%           temp = textscan(flist,'%2f%2f%3f %*2c %f %f %f %*c %f %*[^\n]','delimiter',', ','headerlines',1,'emptyvalue',NaN,'treatAsEmpty','Sonic');
            temp = textscan(flist,'%2f%2f%3f %*3c %*2c %f %f %f %f %*[^\n]','delimiter',', ','headerlines',1,'emptyvalue',NaN,'treatAsEmpty','Sonic');
            st1 = [st1 cell2mat(temp)'];  % 8x~36000 array
        catch
            for ii=1:7  % length of last cell reflects missing values from any/all fields
                if length(temp{1,ii})~=length(temp{1,7})
                    temp{1,ii}(length(temp{1,7})+1) = [];  %truncate length of all cells to length(temp{1,8})
                end
            end
            if ~isempty(temp{1,1})
