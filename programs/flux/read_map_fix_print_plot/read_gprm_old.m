...
%             test = '5545733 $GPRMC,125545.00,A,1449.29376461,N,05103.11997654,W,0.77,195.96,150120,,,A*4E'
            ...
            for ii=1:9      % length of last cell reflects missing values from any/all fields
                 if length(temp{1,ii})~=length(temp{1,9})
                    ...
                    [r,~] = size(temp{1,9});
                    disp(['read error ',dfl,' line ',sprintf('%2i',r+2) ' but delete it in gps file first']);  % +2 to account for the file header
            ...
            if ~isempty(temp{1,1})  % save good values from last read, if any
                ...
                [r,~] = size(temp{1,9});
                error(['read error ',dfl,' line ',sprintf('%2i',r+2) ' but delete it in gps file first']);  % +2 to account for the file header
