function [uvwplat,xyzplat]=accels(bhigh,ahigh,sf,accm,euler)
%
...
% [b,a]=butter(n,wn,'high');
  ...
  gravity = sqrt( sum(gravxyz.^2) );
   ...
   acc      = neaqs_trans(accm,euler,0);    % first rotate  
   acc(3,:) = acc(3,:) - gravity;      % remove gravity
...
% INTEGRATE ACCELERATIONS TO GET PLATFORM VELOCITIES
   ...
   uvwplat(1,:) = (cumsum(acc(1,:)) -.5*acc(1,:) - .5*acc(1,1) )/sf;
   uvwplat(2,:) = (cumsum(acc(2,:)) -.5*acc(2,:) - .5*acc(2,1) )/sf;
   uvwplat(3,:) = (cumsum(acc(3,:)) -.5*acc(3,:) - .5*acc(3,1) )/sf;
