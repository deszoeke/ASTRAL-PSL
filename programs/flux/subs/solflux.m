function [In,Id, Tr, To, Tg, Tw, Ta] = solflux(zdeg,w, p, k1, k2, oz)

%SOLFLUX - COMPUTES GLOBAL SOLAR FLUX, CLEAR SKY

%  [In, Id, Tr, To, Tg, Tw, Ta] = solflux(zdeg, w, p, k1, k2, oz)
...
% oz == ozone-layer thickness in cm(NTP) (optional, default = 0.3)
...
To = 1 - (.1611 .* U3 .* (1+139.48 .* U3) .^ (-.3035) - 0.002715 .* U3 ./(1+.044 .* U3 +.0003 .* U3 .* U3));
...
Idr = 0.79 .* Isc .* mu .* To .* Tg .* Tw .* Taa .* (.5 .* (1-Tr)) ./(1-Ma+Ma .^ 1.02);
