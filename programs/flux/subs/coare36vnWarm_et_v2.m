...
% %%% Update Jan 5, 2022
% This is Simon de Szoeke's change. It increases the magnitude of warm
% layers by about 0.05 deg in PISTON 2019 in the two strongest cases, and
% it increases the depth of the warm layer by about a meter. More testing
% really needs to be done before we move forward with this. Also, I don't
% really understand the math. We really would need to test relative to Ken
% Hughes and Jim Moum's Surf Otter data. 
...
for ibg = 1:nx 			% major read loop discritized by time
    ...
    % Soloviev 3-band solar absorption coefficients tuned from Ohlmann's measurements in DYNAMO
    % see /Users/sdeszoek/Data/cruises/DYNAMO_2011/solar_absorption/test_absorption_fcns.m
    % note: there are many gammas in this program that get rewritten. use caution. 
    F=[0.45 0.27 0.28];       % fraction of sunlight in each band (nondimensional)
    gamma=[12.8 0.357 0.014]; % absorption length scale (meters)
    invgamma=1./gamma;
                ...
                if q_pwp>=50 || jamset==1           %Check for threshold
                    ...
                    if qcol_ac+q_pwp*dtime>0	            %check threshold for warm layer existence
                        ...
                        for i=1:5                           %loop 5 times for fxp
                            ...
                            fxp=1-sum(F.*(exp(-dz_warm*invgamma)),2);   
