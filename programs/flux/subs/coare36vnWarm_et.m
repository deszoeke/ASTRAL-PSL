...
disp('===================================================================')
disp('starting warm layer code for first time')

% if user supplies single value for wave inputs, turn it into an array
if length(sigH) == 1
    new_sigH = repmat(sigH,length(U),1);
    sigH = new_sigH;
end
if length(cP) == 1
    new_cP = repmat(cP,length(U),1);
    cP = new_cP;
end

disp(['num missing jd:    ' sprintf('%i',length(find(isnan(Jd)==1)))  ' / ' sprintf('%i',length(Jd)) ]);
disp(['num missing u:     ' sprintf('%i',length(find(isnan(U)==1)))  ' / ' sprintf('%i',length(U)) ]);
disp(['num missing ta:    ' sprintf('%i',length(find(isnan(Tair)==1)))  ' / ' sprintf('%i',length(Tair)) ]);
disp(['num missing rh:    ' sprintf('%i',length(find(isnan(RH)==1)))  ' / ' sprintf('%i',length(RH)) ]);
disp(['num missing pa:    ' sprintf('%i',length(find(isnan(P)==1)))  ' / ' sprintf('%i',length(P)) ]);
disp(['num missing tsea:  ' sprintf('%i',length(find(isnan(Tsea)==1)))  ' / ' sprintf('%i',length(Tsea)) ]);
disp(['num missing sw:    ' sprintf('%i',length(find(isnan(SW_dn)==1)))  ' / ' sprintf('%i',length(SW_dn)) ]);
disp(['num missing lw:    ' sprintf('%i',length(find(isnan(LW_dn)==1)))  ' / ' sprintf('%i',length(LW_dn)) ]);
disp(['num missing lat:   ' sprintf('%i',length(find(isnan(Lat)==1)))  ' / ' sprintf('%i',length(Lat)) ]);
disp(['num missing lon:   ' sprintf('%i',length(find(isnan(Lon)==1)))  ' / ' sprintf('%i',length(Lon)) ]);
disp(['num missing prate: ' sprintf('%i',length(find(isnan(Rainrate)==1)))  ' / ' sprintf('%i',length(Rainrate)) ]);
disp(['num missing ssea:  ' sprintf('%i',length(find(isnan(Ss)==1)))  ' / ' sprintf('%i',length(Ss)) ]);
disp(['num missing cp:    ' sprintf('%i',length(find(isnan(sigH)==1)))  ' / ' sprintf('%i',length(sigH)) ]);
disp(['num missing sigh:  ' sprintf('%i',length(find(isnan(cP)==1)))  ' / ' sprintf('%i',length(cP)) ]);
...
% disp(size(cP(1)));
% disp(size(U(1)));
disp('starting cool skin code on first value');
...
disp(['doing cool skin code for each time step: ' sprintf('%i',nx) ' total']); 
   
for ibg = 1:nx 			% major read loop discritized by time
% for ibg = 761:763 			% major read loop discritized by time... test a few points
    ...
    cp=Rhoair(ibg);   % air density
    sigh=sigH(ibg);   % wave height
    cp=cP(ibg);   % wave height
...
%     disp(['doing cool skin code for each time step # ' sprintf('%i / %i',ibg,nx)]);

    Bx=coare36vn_zrf_et(u,zu,t,zt,rh,zq,p,ts,sw_dn,lw_dn,lat,lon,jd,zi,rain,ss,cp,sigh,zrf_u,zrf_t,zrf_q);
...
disp('doing cool skin code for all time steps at once with adjusted Tsea, accounting for estimated warm layer between skin and instrument'); 
