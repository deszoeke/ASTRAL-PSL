...
hsig=max(hsig,.25);
ii=find(isfinite(cP) == 1 & isnan(sigH) == 1);
sigH(ii)=hsig(ii);
if ~isempty(ii) 
    disp('recalculating wave height based on wind speed and wave phase speed');
end

% disp(['size sigH ' sprintf('%i',size(sigH))]);
% disp(['size cP ' sprintf('%i',size(cP))]);
% disp(['size u10 ' sprintf('%i',size(u10))]);
% disp(['size ii ' sprintf('%i',size(ii))]);
% disp(['size hsig ' sprintf('%i',size(hsig))]);
...
% disp('starting cool skin iterations');

for i=1:nits
%     disp(['iterations of bulk loop ' sprintf('%i',i)]);
...
%     disp(['N length(u) ' sprintf('%i',N)]);
%     disp(['xlamx ' sprintf('%i',length(xlamx))]);

    dz_skin=min(0.01, visw*xlamx./(sqrt(rhoa./rhow).*usr));
    k=find(alq>0);
    
%     disp(['k (alq > 0) ' sprintf('%i',length(k))]);
    
    [row_alq, col_alq] = size(alq);
    [row_usr, col_usr] = size(usr);
    if row_alq ~= row_usr
        disp(['alq, usr rows: ' sprintf('%i %i %i',[row_alq row_usr])]);
        disp('error, xlamx inputs are not same number of rows');
        disp(['alq, usr cols: ' sprintf('%i %i',[col_alq col_usr])]);
        disp(['alq, usr rows: ' sprintf('%i %i',[row_alq row_usr])]);
        disp(i);
        disp(datestr(datenum(2023,0,jd,0,0,0),0));
    end
    if col_alq ~= col_usr
        disp('error, xlamx inputs are not same number of columns');
        disp(['alq, usr cols: ' sprintf('%i %i',[col_alq col_usr])]);
        disp(['alq, usr rows: ' sprintf('%i %i',[row_alq row_usr])]);
        disp(i);
        disp(datestr(datenum(2023,0,jd,0,0,0),0));
...
%     disp(['length k: ' sprintf('%i',length(k))])
%     disp(['size bigc: ' sprintf('%i %i',size(bigc))]);
%     disp(['size alq: ' sprintf('%i %i',size(alq))]);
%     disp(['size usr: ' sprintf('%i %i',size(usr))]);
%     disp(['size xlamx: ' sprintf('%i %i',size(xlamx))]);
    xlamx(k)=6./((1+ (bigc(k).* alq(k)./usr(k).^4) .^0.75) .^0.333);
    
%     disp(['dz_skin r,c ' sprintf('%i %i',size(dz_skin))]);
%     disp(['xlamx r,c ' sprintf('%i %i',size(xlamx))]);
    
    [row_xlamx, col_xlamx] = size(xlamx);
    [row_rhoa, col_rhoa] = size(rhoa);
%     if row_xlamx ~= row_usr || row_xlamx ~= row_rhoa || row_usr ~= row_rhoa
% %         disp('error, dz_skin inputs are not same number of rows');
% %         disp(['xlamx, usr, rhoa cols: ' sprintf('%i %i %i',[col_xlamx col_usr col_rhoa])]);
% %         disp(['xlamx, usr, rhoa rows: ' sprintf('%i %i %i',[row_xlamx row_usr row_rhoa])]);
% %         disp(i);
% %         disp(datestr(datenum(2023,0,jd,0,0,0),0));
%     end
%     if col_xlamx ~= col_usr || col_xlamx ~= col_rhoa || col_usr ~= col_rhoa
% %         disp('error, dz_skin inputs are not same number of columns');
% %         disp(['xlamx, usr, rhoa cols: ' sprintf('%i %i %i',[col_xlamx col_usr col_rhoa])]);
% %         disp(['xlamx, usr, rhoa rows: ' sprintf('%i %i %i',[row_xlamx row_usr row_rhoa])]);
% %         disp(i);
% %         disp(datestr(datenum(2023,0,jd,0,0,0),0));
%     end
    dzskin_num = visw*xlamx(k);
    dzskin_denom = usr(k).* sqrt(rhoa(k) /rhow);
%     dz_skin(k)=visw*xlamx(k)./(sqrt(rhoa(k)/rhow).*usr(k)); 

%     if length(k) > 1
%         disp(['dzskin num: ' sprintf('%i %i',size(dzskin_num))]);
%         disp(['dzskin den: ' sprintf('%i %i',size(dzskin_denom))]);
%         disp(['k length    ' sprintf('%i',length(k))]);
%     end
 
    dzskin_res = dzskin_num ./ dzskin_denom;
    
%     if length(k) > 1
%         disp(['dzskin res: ' sprintf('%i %i',size(dzskin_res))]);
%     end
    
    dz_skin(k) = dzskin_res;
...
% P is sea level pressure, so use subtraction through hydrostatic eqfuation 
...
% disp('doing white cap fraction');
% disp('starting cool skin waves');
...
% if length(kk) > 1
%     disp(['kk ' sprintf('%i %i',size(kk))]);
%     disp(['wc_frac(kk) ' sprintf('%i %i',size(wc_frac(kk)))]);
%     disp(['U10N(kk) ' sprintf('%i %i',size(U10N(kk)))]);
%     disp(['cP(kk) ' sprintf('%i %i',size(cP(kk)))]);
% 
%     lhs = wc_frac(kk);
%     rhs = 1.6e-3*U10N(kk).^1.1./sqrt(cP(kk)./U10N(kk));  
%     disp(['lhs ' sprintf('%i %i',size(lhs))]);
%     disp(['rhs ' sprintf('%i %i',size(rhs))]);
%     disp(['lhs ' sprintf('%i',length(lhs))]);
%     disp(['rhs ' sprintf('%i',length(rhs))]);
% end

% I don't know why cP is flipped, but it is? 
[row_cp, col_cp] = size(cP);
[row_U10N, col_U10N] = size(U10N);

% disp(['U10N   ' sprintf('%i %i',size(U10N))]);

if row_cp == col_U10N && row_U10N ~= 1
    cpp = cP';
    disp('flipping cP');
%     disp(['cpp    ' sprintf('%i %i',size(cpp))]);
    denom = sqrt(cpp(kk)./U10N(kk));
%     disp(['denom ' sprintf('%i %i',size(denom))Index exceeds the number of array elements (1).
else
%     disp(['cP     ' sprintf('%i %i',size(cP))]);
    denom = sqrt(cP(kk)./U10N(kk));
%     disp(['denom ' sprintf('%i %i',size(denom))]);
end

[row_denom, col_denom] = size(denom);
if row_denom == col_denom && col_denom ~= 1 && col_denom ~= 0
    disp('warning: 2D vector instead of simple row');
    disp(['sqrt(cP/u10n) ' sprintf('%i %i',size(denom))]);
    disp(['cP ' sprintf('%i %i',size(cP))]);
    disp(['U10N ' sprintf('%i %i',size(U10N))]);
end

numer = 1.6e-3*U10N(kk).^(1.1);

res = numer./denom; 
% disp(['result ' sprintf('%i %i',size(res))]);

wc_frac(kk) = res;
% if length(kk) > 1
%     disp(['denom ' sprintf('%i %i',size(denom))]);
%     disp(['num ' sprintf('%i %i',size(numer))]);
%     disp(['res ' sprintf('%i %i',size(res))]);
%     disp(['denom ' sprintf('%i',length(denom))]);
%     disp(['num ' sprintf('%i',length(numer))]);
%     disp(['res ' sprintf('%i',length(res))]);
% end
% wc_frac(kk)= 1.6e-3*U10N(kk).^(1.1) ./ sqrt(cP(kk)/U10N(kk)); 

% disp('doing wave energy dissipation');
Edis=0.095*rhoa.*U10N.*usr.^2;  %  energy dissipation rate from breaking waves W/m^2
wc_frac(iice)=0;Edis(iice)=0;

%****************  output  ****************************************************
% get rid of default 0.01 dz_skin values which are non-physical and only
% happen for bad data anyway
bad_skin = find(dz_skin > 0.009);
% disp(['bad dz_skin = ' sprintf('%i',length(bad_skin))]);
dT_skin(bad_skin) = nan;
dz_skin(bad_skin) = nan;
dq_skin(bad_skin) = nan;
...
% disp(['bad u = ' sprintf('%i',length(bad_input))]);
gust(bad_input) = nan;
