function x2 = interval_std_var(t1, x1, t2)
%{
Computes std dev for high-rate data over intervals t2.

t1 and t2 are assumed to be 1-D.  x1 may be 2-D, in which case
the std devs are computed along columns, which must have the same
...
NO STD WILL BE COMPUTED FOR THIS VALUE.  THE RESULT ARRAY WILL
...
if (test_t1 || test_t2)
    error('interval_std: time arrays must be 1-D');
end
if length(ndims(x1))>2
    error('interval_std: data array more than 2-D');
end
if size(x1,1)==1 && size(x1,2)>1
    x1 = x1'; % if x1 is 1-D row, convert to col
end
if length(t1) ~= size(x1,1)
    error('interval_std: t1 and x1 not equal length');
end
if mean(diff(t2)) <= mean(diff(t1))
    error('interval_std: delta t2 should be > delta t1');
end
if t2(1) > t1(end) || t2(end) < t1(1)
    error('interval_std: intervals do not overlap');
end
...
% do not add timestamp as first col of output array
% if you want timestamp, then use int_avg instead
% x2 = [t2(1:end-1),x2];
