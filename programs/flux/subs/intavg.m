function x=intavg(tt,xx,t)
% intavg(tt,xx,t);
% Average xx(tt) over new time intervals t. tt and t are the beginning
% of time intervals. The average conserves integrals.
...
for i=sti:ni-1
    ...
    s=0; dts=0;
    while (tt(ii)<t(i+1) && ii<nii )
        if isfinite(xx(ii))
            dt=min(tt(ii+1),t(i+1))-max(tt(ii),t(i));
            s=s+xx(ii)*dt;
            dts=dts+dt;
        end
        ii=ii+1;
...
i=ni; % t(i+1) does not exist
    ii=find(tt<t(i),1,'last');
    s=0; dts=0;
    while (ii<nii && isfinite(xx(ii)))
        dt=tt(ii+1)-max(tt(ii),t(i));
        s=s+xx(ii)*dt;
        dts=dts+dt;
        ii=ii+1;
    end
