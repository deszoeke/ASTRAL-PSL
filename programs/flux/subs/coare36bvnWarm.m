function B=coare36bvnWarm(Jd,U,zu,Tair,zt,RH,zq,P,Tsea,SW_dn,LW_dn,Lat,Lon,zi,Rainrate,ts_depth,Ss,cp,sigH,zrf_u,zrf_t,zrf_q)
...
%     Tsea = subsurface seawater temperature (degC) at ts_depth
%	 SW_dn = downward solar flux (w/m^2) defined positive down
%	 LW_dn = downward IR flux (w/m^2) defined positive down
%	   Lat = latitude (deg N = positive)
%	   Lon = longitude (deg E = positive) % If using degrees west, see
%               usage of lon in albedo_vector function and adjust 'E' to
%               'W' string input
%       zi = inversion height (m), assumed to be 600 m
% Rainrate = rain rate (mm/hr)
% ts_depth = depth (m) of water temperature measurement, positive for below
% surface
%       Ss = sea surface salinity (PSU), or value closest to surface as
%       possible
...
%********** output data  ***************
% Outputs
% Adds onto output from coare36bvn_zrf
...
% for clarity:
%    * dT_warm = dT from base of warm layer to skin, i.e. warming across
%         entire warm layer depth (deg C). positive when warm layer is
%         present.
%    * dz_warm = warm layer thickness (m)
%    * dT_warm_to_skin = dT from measurement depth to skin due to warm layer, 
%          such that skin level sea surface temperature, Tskin, equals... 
...
Bx=coare36bvn_zrf(U(1),zu(1),Tair(1),zt(1),RH(1),zq(1),P(1),Tsea(1),SW_dn(1),LW_dn(1),Lat(1),Lon(1),Jd(1),zi(1),Rainrate(1),Ss(1),cp(1),sigH(1),zrf_u(1),zrf_t(1),zrf_q(1));
...
hrain_old=Bx(36);  %rain heat flux
...
% note: in prior COARE lingo/code: dT_warm_to_skin used to be dsea and dT_skin used to be dter
    Bx=coare36bvn_zrf(u,zu,t,zt,rh,zq,p,ts,sw_dn,lw_dn,lat,lon,jd,zi,rain,ss,cp,sigH,zrf_u,zrf_t,zrf_q);
...
Bx=coare36bvn_zrf(U,zu,Tair,zt,RH,zq,P,Tsea,SW_dn,LW_dn,Lat,Lon,Jd,zi,Rainrate,Ss,cp,sigH,zrf_u,zrf_t,zrf_q);
