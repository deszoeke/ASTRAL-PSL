function y = prctilex(x,p,dim)
%prctilex Percentiles of a sample.
%   Y = PRCTILEX(X,P) returns percentiles of the values in X.  P is a scalar
...
%   For N-D arrays, prctilex operates along the first non-singleton
%   dimension.
%
%   Y = prctilex(X,P,DIM) calculates percentiles along dimension DIM.  The
%   DIM'th dimension of Y has length LENGTH(P).
%
%   Percentiles are specified using percentages, from 0 to 100.  For an N
%   element vector X, prctilex computes percentiles as follows:
...
%
%   prctilex treats NaNs as missing values, and removes them.
%
%   Examples:
%      y = prctilex(x,50); % the median of x
%      y = prctilex(x,[2.5 25 50 75 97.5]); % a useful summary of x
%
%   See also IQR, MEDIAN, nanmedian1, QUANTILE.

%   Copyright 1993-2004 The MathWorks, Inc.
%   $Revision: 2.12.4.5 $  $Date: 2004/06/25 18:52:56 $

if ~isvector(p) || numel(p) == 0
    error('stats:prctilex:BadPercents', ...
...
% Figure out which dimension prctilex will work along.
sz = size(x);
if nargin < 3 
    dim = find(sz ~= 1,1);
    if isempty(dim)
