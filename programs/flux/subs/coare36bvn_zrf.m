function A=coare36bvn_zrf(u,zu,t,zt,rh,zq,P,ts,sw_dn,lw_dn,lat,lon,jd,zi,rain,Ss,cp,sigH,zrf_u,zrf_t,zrf_q)
%

%**************************************************************************
% VERSION INFO:

% Vectorized version of COARE version 3.6b code (Fairall et al, 2003) with 
% modification based on the CLIMODE, MBL and CBLAST experiments 
% (Edson et al., 2013). The cool skin and surface wave options are included.
% A separate warm layer function can be used as the "driver" program, such 
% that it will call this cool skin function.
%
% This 3.6b version includes parameterizations using wave height and wave
...
%             sensor depth and whether warm layer is computed. ts is
%             assumed to be the subsurface or subskin temperature if jcool
%             = 1. 
