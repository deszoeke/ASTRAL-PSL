function [sw,sz,saz,dir,sky] = SolarRadiance(alat,alon,jd, tutc,watvap, p,k1, k2, oz)

% SOLARRADIANCE --	Radince, zenith, and azimuth
% [sw,sz,saz,dir,sky] = SolarRadiance(alat,alon,jd, tutc,watvap, p, k1, k2, oz)
...
%  watvap = integrated water vapor (kg/m^2) -- use 5 for TWP, default = 3.
%  p = pressure, default = 1013 hPa
%  k1, k2 = aerosol absorbtion coefs, default = .3
%  oz = ozone thickness, default = 0.3;
...
% TESTING
% alat = 0;  alon = 162;  jd = 180;%julian(1997,2,5);
% tutc = [0:.5:24]';
% watvap = 5;
...
te = te .* te;
alpha = alambda - r2d .* te .* sin( 2 * rlambda ) +	0.5 .* r2d .* te .* te .* sin(4 * rlambda);
...
% Correct for E-W
% AZIMUTH ANGLE
s_az = ( cos(pi * 0.5 - solar_dec) - cos(colatr) .* c_solar_z)./ ( sin(colatr) .* sin(s_z) );
saz = acos(s_az) * r2d;
...
if watvap(1) < 0
   ...
   [dir,sky] = solflux(sz,watvap,p,k1,k2,oz);
